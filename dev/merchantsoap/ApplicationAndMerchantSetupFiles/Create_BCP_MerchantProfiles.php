...
if ($_merchantProfileId == null){
	...
	if (isset($_serviceId[1])){
		...
		foreach ($_serviceId as $svcId){
			...
			if ($svcId['ServiceId'] == 'C82ED00001'){ //TSYS Sierra
				$merchProfile->MerchantData->BankcardMerchantData->TerminalId = '0004';
				$merchProfile->MerchantData->BankcardMerchantData->AgentBank = '120004';
				$merchProfile->MerchantData->BankcardMerchantData->Location = '00001';
				$merchProfile->MerchantData->BankcardMerchantData->AcquirerBIN = '800001';
				$merchProfile->MerchantData->BankcardMerchantData->SecondaryTerminalId = '70070004';
			}
			if ($svcId['ServiceId'] == '4365400001' || $svcId['ServiceId'] == '6781E00001'){// Vantiv
				$merchProfile->MerchantData->BankcardMerchantData->TerminalId = '004';
				$merchProfile->MerchantData->BankcardMerchantData->AgentBank = '1204';
				$merchProfile->MerchantData->BankcardMerchantData->AgentChain = '8704';
				$merchProfile->MerchantData->BankcardMerchantData->Location = '001';
				$merchProfile->MerchantData->BankcardMerchantData->AcquirerBIN = '4001';
				$merchProfile->MerchantData->BankcardMerchantData->ABANumber = '4021';
				$merchProfile->MerchantData->BankcardMerchantData->SecondaryTerminalId = '704';

			}
			if ($svcId['ServiceId'] == 'E88FD00001'){ //TSYS Summit
				$merchProfile->MerchantData->BankcardMerchantData->AgentBank = 'AF1204';
				$merchProfile->MerchantData->BankcardMerchantData->TerminalId = '835620984198704';
				$merchProfile->MerchantData->BankcardMerchantData->AcquirerBIN = '204001';

			}
			if ($svcId['ServiceId'] == 'A4F2B00001'){ //Salem Direct
				$merchProfile->MerchantData->MerchantId = '123456';
			}
			if ($svcId['ServiceId'] != '35A7700001')
			$mpXML = createMerchantProfileXML($merchProfile);
			$response = $client->saveMerchantProfiles ( $mpXML, 'Credit', $svcId['ServiceId'] );
		}
	}
	else{
		$merchProfile = setBCPMerchantProfile();
		...
		$mpXML = createMerchantProfileXML($merchProfile);
		$response = $client->saveMerchantProfiles ( $mpXML, 'Credit', $_serviceId['singleService']['ServiceId'] );
	}
}
...
if ($_merchantProfileId != null){
	...
	if (is_array($_merchantProfileId))
	{
		...
		foreach($_merchantProfileId as $id){
			...
			if ($response)
			{
				echo '<li>'.$id['ProfileId'].' with ServiceId of:</b><font color="#800080"> '.$id['ServiceId'].' is initialized.<br/></font></li>';
			}
...
function setBCPMerchantProfile(){
	...
	$bankCardMerchData->ABANumber = '128965';
	// <!--type: string--> Optional - Required for FTPS and TSYS
	$bankCardMerchData->AgentChain = '128965';
	// <!--type: string--> Optional - Required for FTPS and TSYS
	$bankCardMerchData->AgentBank = '1289';
	// <!--type: string--> Optional - Required for FTPS and TSYS
	$bankCardMerchData->AcquirerBIN = '1248';
	// <!--type: string--> Optional - Required for FTPS and TSYS
	$bankCardMerchData->Location = '108';
	...
	// <!--type: string--> Optional - Required for TSYS
	$bankCardMerchData->TimeZoneDifferential = '750';
	...
	// <!--type: string-->
	$bankCardMerchData->ClientNumber = '1234';
	// <!--type: boolean-->
